...
import { ArrowLeft, Save, Info, DollarSign, Image as ImageIcon, Package, Tag, Eye } from 'lucide-react';
...
export default function EditProductPage() {
  ...
  const productId = params.id as string; // Captura el ID dinámico de la URL

  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [restaurants, setRestaurants] = useState<Restaurant[]>([]);

  // Estados del formulario mapeados al modelo relacional
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        if (!restaurantsRes.ok) throw new Error('No se pudieron cargar los restaurantes');
        ...
        // Pre-rellenamos los estados con la data real de PostgreSQL
        setRestaurantId(productData.restaurantId);
        setName(productData.name);
        setDescription(productData.description || '');
        setCategory(productData.category);
        setCalories(productData.calories ? productData.calories.toString() : '');
        setPrice(productData.price.toString());
        setImageUrl(productData.image);
        setIsAvailable(productData.isAvailable);
        setIsPopular(productData.isPopular);

      } catch (error) {
        console.error('Error de QA en precarga:', error);
        toast.error('Error al recuperar los datos del producto');
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen bg-[#F8FAFC] antialiased text-gray-400 justify-center items-center font-sans">
        <div className="text-center space-y-2">
          <p className="text-sm font-bold animate-pulse text-gray-500">Sincronizando con PostgreSQL...</p>
          <p className="text-xs text-gray-400">Recuperando propiedades del producto {productId}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex min-h-screen bg-[#F8FAFC] font-sans antialiased text-gray-900 w-full">
      ...
      <main className="flex-1 p-6 sm:p-8 max-w-5xl space-y-6 overflow-y-auto">
        ...
        <div className="flex items-center gap-4 pt-1">
          ...
          <div>
            <h1 className="text-3xl font-black text-gray-950 tracking-tight">Modificar Producto</h1>
            <p className="text-gray-400 text-xs font-medium mt-0.5">Edita las propiedades guardadas en PostgreSQL</p>
          </div>
        </div>
